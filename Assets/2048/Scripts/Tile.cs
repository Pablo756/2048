...
public class Tile : MonoBehaviour {
	...
	public bool Move (int x, int y)
    {
		...
		if(transform.position != (Vector3)movePosition)
        {
			...
			Manager.grid[Mathf.RoundToInt(Manager.worldToGrid(transform.position.x,transform.position.y).x),
                         Mathf.RoundToInt(Manager.worldToGrid(transform.position.x,transform.position.y).y)] = null;
			return true;
